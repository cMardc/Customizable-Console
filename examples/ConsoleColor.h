...
#ifndef CUSTOM_CONSOLE__
#define CUSTOM_CONSOLE__
#pragma once
 ...
 */
void c_setAttribute(const char* attribute) { printf("%s", attribute); }
 ...
 * @return This function does not return anything
 */
void c_reset() { printf(A_RESET); }

/*
 * @brief (Linux) Set more than one Attribute
 * @param attributes A string (const char*) parameter where all attributes must
 * be stored
 * @param length A number (const size_t) specifies length of first argument /
 * parameter
 * @return This function does not return anything
 */
...
/*
 * @brief (Linux) Function to set the cursor position to an absolute row and
 * column
 * @param row This argument must be a number (const int) and it sets row for
 * cursor
 * @param col This argument must be a number (const int) and it sets column for
 * cursor
 * @return This function does not return anything
 * @note This function is Linux only
 */
 ...
 * @note This function is Linux only
 */
void c_moveToRow(const int row) { c_setCursorPosition(row, 1); }

/*
 * @brief (Linux) Function to move the cursor to the beginning of a specific
 * column
 * @param col This argument must be a number (const int) and it sets column for
 * cursor
 * @return This function does not return anything
 * @note This function is Linux only
 */
void c_moveToColumn(const int col) { c_setCursorPosition(1, col); }
 ...
 */
void c_alertSound() { printf("%c", E_ALERT); }
 ...
 * @param width This argument must be a number (const int) and it specifies a
 * width for line
 ...
 * @param width This argument must be a number (const int) and it specifies
 * width for box
 * @param height This argument must be a number (const int) and it specifies
 * height for box
 * @param crossChar This argument must be a character (const char) and it will
 * be printed on the corners of box
 * @param horizontalChar This argument must be a character (const char) and it
 * will be printed on top and bottom of box
 * @param verticalChar This argument must be a character (const char) and it
 * will be printed on the right and left of box
 * @return This function does not return anything
 * @note This function outputs box directly without asking
 */
void c_draw_box(const int width, const int height, const char crossChar,
                const char horizontalChar, const char verticalChar) {
  printf("%c", crossChar);
 ...
 * @param verticalChar This argument must be a character (const char) and it
 * will be printed on the right and left of box
 * @param text This argument must be a string (const char*) and it will be on
 * the center og box
 * @return This function does not return anything
 * @note This function outputs box directly without asking
 */
void c_draw_text_box(const int height, const char crossChar,
                     const char horizontalChar, const char verticalChar,
                     const char* text) {
  int textPoint = -1;
 ...
 * @param radius This argument must be a number (const int) and it specifies
 * radius for circle
 * @return This function does not return anything
 ...
 * @param height This argument must be a number (const int) and it specifies
 * height for triangle
 * @param text This argument must be a string (const char*) and it will be
 * printed on the center of triangle
 * @return This function does not return anything
...
/*
 * @brief Function to draw a table
 * @param tableData two dimensional string array that will be printed (const
 * char***)
 * @param numRows number of rows in table (int)
 * @param numCols number of columns in table (int*)
 */
void c_printTable(const char*** tableData, int numRows, int* numCols) {
  ...
  int totalWidth = 0;  // Start with 1 for the left border
  for (int i = 0; i < *numCols; ++i) {
    totalWidth +=
        colWidths[i] + 3;  // Add 3 for cell content, right border, and space
  }
  ...
  printf("| ");
  for (int i = 0; i < *numCols; ++i) {
    printf("%-*s | ", colWidths[i], tableData[0][i]);
  }
  printf("\n");
  ...
  for (int i = 1; i < numRows; ++i) {
    ...
    for (int j = 0; j < *numCols; ++j) {
      printf("%-*s | ", colWidths[j], tableData[i][j]);
    }
    printf("\n");
  }
